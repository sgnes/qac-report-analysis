...
class QacReportParser(object):
    ...
    def get_all_errors(self):
        ...
        for i in m:
            error_line_num = i[0]
            error_text = i[1]
            error_dict[error_line_num] = error_text
            error_detail = re.findall(r"\+\+\s([A-Z\s]{7,10})\s\+\+: &lt;=\d=\((\d{4}\))(.*?)</A>", error_text)
            #self.__logger.info("Error line number:{0}".format(i[0]))
            error_line_nums = error_line_nums + "," + i[0]
        self.__logger.info("File:{0} Error Lines:{1}".format(self.__html_file, error_line_nums))
        return error_dict
